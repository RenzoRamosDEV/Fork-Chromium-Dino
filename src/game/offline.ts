...
// Claves de cadenas de accesibilidad en loadTimeData.
// Se usan para anunciar eventos del juego a lectores de pantalla.
enum A11yStrings {
...
// Valores base compartidos entre el modo normal y el lento.
...
// Configuración del modo normal: más rápido, saltos normales, más obstáculos.
...
// Configuración del modo lento (a11y): menor velocidad y aceleración, gaps mayores.
...
// Clases CSS que Runner añade/quita al DOM para reflejar el estado del juego.
enum RunnerClasses {
...
// IDs de los elementos <audio> en el HTML (dentro del <template id="audio-resources">).
// Runner los decodifica a AudioBuffer al iniciar el primer juego.
enum RunnerSounds {
...
// Códigos de tecla que controlan el juego.
// jump: ↑ (38) y Espacio (32) — saltar o iniciar el juego.
// duck: ↓ (40) — agacharse o hacer speed-drop en el aire.
// restart: Enter (13) — reiniciar tras game over.
...
// Singleton de Runner; se crea una sola vez con initializeInstance().
let runnerInstance: Runner|null = null;

// Si el título de la página empieza con esta URL, el juego se muestra en modo
// pantalla completa (chrome://dino) sin el texto de error.
const ARCADE_MODE_URL: string = 'chrome://dino/';

// Prefijo de los IDs de las imágenes del sprite en el HTML: "offline-resources-1x" / "offline-resources-2x".
const RESOURCE_POSTFIX: string = 'offline-resources-';

// Controlador principal del juego. Es un singleton que implementa cuatro
// interfaces para que los subcomponentes accedan a él sin acoplamiento directo:
//   ImageSpriteProvider  → expone los sprites cargados
//   GameStateProvider    → expone flags de estado (slowdown, audioCues, altMode)
//   ConfigProvider       → expone la configuración activa
//   GeneratedSoundFxProvider → expone el motor de audio procedural
//
// Ciclo de vida:
//   initializeInstance() → loadImages() → init() → [loop: update() vía rAF]
export class Runner implements ImageSpriteProvider, GameStateProvider,
                               ConfigProvider, GeneratedSoundFxProvider {
  // El div padre que recibe el canvas (.interstitial-wrapper en la página de error).
  private outerContainerEl: HTMLElement;
  // El div runner-container creado dinámicamente; contiene el canvas y elementos a11y.
  private containerEl: HTMLElement|null = null;
  // Div transparente sobre toda la pantalla para capturar touch en móvil.
  private touchController: HTMLElement|null = null;
  private canvas: HTMLCanvasElement|null = null;
  private canvasCtx: CanvasRenderingContext2D|null = null;
  // Span aria-live para anunciar eventos a lectores de pantalla.
  private a11yStatusEl: HTMLElement|null = null;
  // Label del checkbox de velocidad lenta (visible solo con a11y activado).
  ...
  // Sprite sheet original siempre disponible (usado por GameOverPanel para el restart).
  private origImageSprite: HTMLImageElement|null = null;
  // Sprite sheet de elementos comunes del modo alternativo.
  private altCommonImageSprite: HTMLImageElement|null = null;
  // Sprite sheet específico del modo alternativo activo.
  private altGameImageSprite: HTMLImageElement|null = null;
  // Sprite sheet activo: cambia al activar el modo alt.
  private imageSprite: HTMLImageElement|null = null;

  private config: Config;
  // Dimensiones lógicas del canvas (pueden ser menores que DEFAULT_DIMENSIONS en móvil).
  private dimensions: Dimensions = DEFAULT_DIMENSIONS;
  // Tipo del modo alternativo activo (null = modo original).
  private gameType: (keyof SpriteDefinitionByType|null) = null;
  // Definición de sprites del modo activo (posiciones, obstáculos, líneas...).
  private spriteDefinition: SpriteDefinition = spriteDefinitionByType.original;
  // Posiciones del sprite sheet activo (ldpi o hdpi según IS_HIDPI).
  private spriteDef: SpritePositions|null = null;

  // true cuando el modo alternativo está activo y sus assets cargaron bien.
  private altGameModeActive: boolean = false;
  // Temporizador del parpadeo del dino al cambiar de modo; null cuando no está parpadeando.
  private altGameModeFlashTimer: number|null = null;
  // true si los assets del modo alt fallaron al cargar (lo desactiva).
  private altGameAssetsFailedToLoad: boolean = false;
  // Acumulador para el fade-in del canvas al activar el modo alt.
  ...
  // Timestamp del frame anterior (para calcular deltaTime).
  private time: number = 0;
  // Distancia acumulada en píxeles desde el inicio de la partida.
  private distanceRan: number = 0;
  // Tiempo total transcurrido desde el inicio de la partida (ms); controla clearTime.
  private runningTime: number = 0;
  private currentSpeed: number;
  // ID del setInterval de debounce del resize.
  private resizeTimerId?: number;
  // ID del requestAnimationFrame activo; 0 = loop parado.
  private raqId: number = 0;
  // Número de partidas jugadas desde que se cargó la página.
  private playCount: number = 0;

  // true si el dispositivo es CrOS con política que deshabilita el easter egg.
  private isDisabled: boolean = loadTimeData.valueExists('disabledEasterEgg');
  // true tras el primer salto (dispara la animación de intro).
  private activated: boolean = false;
  // true mientras el juego está en marcha (no pausado ni game over).
  private playing: boolean = false;
  private playingIntro: boolean = false;
  private crashed: boolean = false;
  private paused: boolean = false;
  // true cuando los colores están invertidos (modo noche activo).
  private inverted: boolean = false;
  private isDarkMode: boolean = false;
  // true cuando hay un rAF pendiente (evita encolar múltiples frames).
  private updatePending: boolean = false;
  private hasSlowdownInternal: boolean = false;
  private hasAudioCuesInternal: boolean = false;

  // Récord de la sesión en píxeles recorridos.
  private highestScore: number = 0;
  // true cuando el récord está sincronizado con el perfil de Chrome.
  private syncHighestScore: boolean = false;

  // Tiempo acumulado en ms desde que empezó la inversión de colores actual.
  private invertTimer: number = 0;
  // true cuando se debe activar la inversión de colores en este tick.
  private invertTrigger: boolean = false;

  // AudioBuffers decodificados de los mp3; se rellenan de forma asíncrona al inicio.
  private soundFx:
      Partial<{[K in keyof typeof RunnerSounds]: AudioBuffer}> = {};
  private audioContext: AudioContext|null = null;
  // Motor de sonido procedural; solo se crea cuando hasAudioCues es true.
  private generatedSoundFx: GeneratedSoundFx|null = null;

  // Estado del gamepad: índice del gamepad activo y snapshot del frame anterior.
