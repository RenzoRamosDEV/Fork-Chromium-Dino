...
// Duración total de la animación del botón de reinicio (ícono giratorio), en ms.
const RESTART_ANIM_DURATION: number = 875;
// Tiempo de pausa antes de que empiece a girar el botón de reinicio, en ms.
const LOGO_PAUSE_DURATION: number = 875;
// Número de veces que parpadea el texto alternativo antes de volver al original.
...
// Panel que se muestra al perder: texto "GAME OVER" + botón de reinicio giratorio.
// El botón aparece después de LOGO_PAUSE_DURATION ms y gira en 8 frames.
// En modo alternativo el texto puede parpadear alternando con un texto especial.
export class GameOverPanel {
  private canvasCtx: CanvasRenderingContext2D;
  private canvasDimensions: Dimensions;
  // Posición del texto "GAME OVER" en el sprite sheet.
  private textImgPos: SpritePosition;
  // Posición del ícono de reinicio (flecha circular) en el sprite sheet.
  private restartImgPos: SpritePosition;
  private imageSpriteProvider: ImageSpriteProvider;
  // Posición del adorno extra del modo alternativo (nulo si es el modo original).
  private altGameEndImgPos: SpritePosition|null;
  private altGameModeActive: boolean;
  private frameTimeStamp: number = 0;
  // Acumulador de tiempo para avanzar el frame de animación del botón.
  private animTimer: number = 0;
  // Frame actual de la animación del botón de reinicio (0-7).
  private currentFrame: number = 0;
  // rAF ID del loop de animación del panel; null cuando el panel está detenido.
  private gameOverRafId: number|null = null;
  private flashTimer: number = 0;
  private flashCounter: number = 0;
  // true = mostrar texto original; false = mostrar texto del modo alt.
