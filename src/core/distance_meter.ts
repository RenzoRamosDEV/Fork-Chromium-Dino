...
// Marcador de distancia recorrida que se muestra en la esquina superior derecha.
// Cada dígito se extrae individualmente del sprite sheet (posición = valor * anchoDígito).
// Al alcanzar múltiplos de 100, el marcador parpadea como celebración.
// El récord "HI" se guarda en highScore y se muestra con 80% de opacidad.
// El usuario puede hacer clic sobre el récord para resetearlo (doble clic confirmado).
export class DistanceMeter {
  // true mientras se reproduce la animación de logro (parpadeo del marcador).
  ...
  // Posición X calculada para alinear el marcador a la derecha del canvas.
  private x: number = 0;
  private y: number = 5;
  // Valor numérico máximo representable con maxScoreUnits dígitos.
  private maxScore: number = 0;
  // Cadena "HI XXXXX" con el récord; vacía si no hay récord.
  private highScore: string = '0';
  // Array de caracteres del marcador actual para dibujarlos dígito a dígito.
  private digits: string[] = [];
  // Cadena de ceros por defecto ("00000") para rellenar con leading zeros.
  private defaultString: string = '';
  private flashTimer: number = 0;
  private flashIterations: number = 0;
  // rAF ID del loop de parpadeo del récord al hacer clic.
  private flashingRafId: number|null = null;
  // Bounding box del área clicable del récord para detectar clicks.
  private highScoreBounds: CollisionBox|null = null;
  private highScoreFlashing: boolean = false;
  // Número de dígitos activos; se amplía a 6 si la puntuación supera 99999.
