...
// Escenario completo: gestiona la línea del suelo, las nubes, los obstáculos
// y el modo noche. Es el componente más grande del juego después de Runner.
// En cada tick Runner llama a horizon.update() que actualiza todos los subelementos.
export class Horizon {
  // Lista de obstáculos activos en pantalla; Runner la lee para detectar colisiones.
  obstacles: Obstacle[] = [];
  ...
  private resourceProvider: ImageSpriteProvider&ConfigProvider&
      GameStateProvider;
  // Historial de los últimos tipos de obstáculos generados (evita repetición excesiva).
  ...
  // Tipos de obstáculos disponibles en el modo actual (se puede cambiar con enableAltGameMode).
  private obstacleTypes: ObstacleType[] = [];

  // Nubes del fondo; se crean y eliminan dinámicamente manteniendo máximo MAX_CLOUDS.
  private clouds: Cloud[] = [];
  private cloudSpeed: number;

  // Elementos de fondo para modos alternativos (reemplazan a las nubes en esos modos).
  private backgroundEls: BackgroundEl[] = [];
  // Tipo del último elemento de fondo añadido (evita repetir el mismo tipo seguido).
  private lastEl: string|null = null;

  // Los segmentos del suelo; normalmente uno, pero puede haber varios en modos alt.
