...
// Multiplicador del gap mínimo para calcular el gap máximo (variación aleatoria).
// maxGap = minGap * maxGapCoefficient.
let maxGapCoefficient: number = 1.5;

// Máximo de obstáculos del mismo tipo que pueden aparecer agrupados.
...
// Un obstáculo individual (cactus pequeño, cactus grande, pterodáctilo, coleccionable).
// Su tipo determina tamaño, posición Y, velocidad y cajas de colisión.
// Se crea siempre fuera del canvas por la derecha y se desplaza hacia la izquierda.
export class Obstacle {
  // Cajas de colisión clonadas de typeConfig (pueden modificarse según el tamaño).
  collisionBoxes: CollisionBox[] = [];
  // true cuando ya se ha creado el siguiente obstáculo en la cola (evita crear duplicados).
  followingObstacleCreated: boolean = false;
  // Espacio en píxeles que debe quedar libre antes del siguiente obstáculo.
  gap: number = 0;
  // true cuando ya se emitió el aviso de audio de proximidad para este obstáculo.
  jumpAlerted: boolean = false;
  // Cuando es true, Horizon lo elimina del array en el siguiente tick.
  remove: boolean = false;
  // Cuántos obstáculos del mismo tipo aparecen agrupados (1-maxObstacleLength).
  size: number;
  // Ancho total = typeConfig.width * size.
  width: number = 0;
  xPos: number;
  yPos: number = 0;
  ...
  // Diferencia de velocidad respecto al horizonte (solo pterodáctilo; aleatorio ±speedOffset).
  private speedOffset: number = 0;
  private altGameModeActive: boolean;
  private imageSprite: CanvasImageSource;
  // Frame actual para obstáculos animados (pterodáctilo tiene 2 frames).
  private currentFrame: number = 0;
  // Acumulador de tiempo para avanzar el frame de animación.
